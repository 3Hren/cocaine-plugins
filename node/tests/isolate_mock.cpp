...
struct spooled_dispatch_t : public dispatch<io::isolate_spooled_tag> {
    spooled_dispatch_t() : dispatch<io::isolate_spooled_tag>("spooled_isolate") {
        on<io::isolate_spooled::cancel>([]() { COCAINE_LOG_INFO(log, "spool::cancel"); });
    }
};

struct spawned_dispatch_t : public dispatch<io::isolate_spawned_tag> {
    spawned_dispatch_t() : dispatch<io::isolate_spawned_tag>("spawned_isolate") {
        on<io::isolate_spawned::kill>([]() { COCAINE_LOG_INFO(log, "spawn::kill"); });
    }
};

struct spool_slot_t : public io::basic_slot<io::isolate::spool> {
public:
    typedef typename io::aux::protocol_impl<
        typename io::event_traits<io::isolate::spool>::upstream_type>::type protocol;
    typedef typename io::basic_slot<io::isolate::spool>::dispatch_type dispatch_type;

    virtual boost::optional<std::shared_ptr<const dispatch_type>> operator()(
        tuple_type&& args,
        ...
        return boost::optional<std::shared_ptr<const dispatch_type>>(
            std::make_shared<spooled_dispatch_t>());
    }
};

struct spawn_slot_t : public io::basic_slot<io::isolate::spawn> {
public:
    typedef typename io::aux::protocol_impl<
        typename io::event_traits<io::isolate::spawn>::upstream_type>::type protocol;
    ...
    virtual boost::optional<std::shared_ptr<const dispatch_type>> operator()(
        ...
        return boost::optional<std::shared_ptr<const dispatch_type>>(
            std::make_shared<spawned_dispatch_t>());
    }
};

class isolate_mock_t : public api::service_t, public dispatch<io::isolate_tag> {
public:
    isolate_mock_t(context_t& context,
                   asio::io_service& asio,
                   const std::string& name,
                   const dynamic_t& args)
        : api::service_t(context, asio, name, args), dispatch<io::isolate_tag>(name) {
    ...
    virtual auto prototype() const -> const io::basic_dispatch_t& {
        return *this;
    }
