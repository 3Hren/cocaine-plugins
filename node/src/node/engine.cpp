...
engine_t::engine_t(context_t& context, profile_t profile, manifest_t manifest,
...
dynamic_t::object_t
engine_t::info(io::node::info::flags_t flags) const {
    dynamic_t::object_t result;

    result["uptime"] = uptime().count();

    const auto profile = (*this->profile.synchronize());
...
engine_t::enqueue(io::streaming_slot<io::app::enqueue>::upstream_type downstream,
                    app::event_t event,
    ...
    boost::optional<slave::id_t> id) -> std::shared_ptr<api::stream_t>
{
    ...
    count_success_if(&stats.requests.accepted, &stats.requests.rejected, [&] {
        queue.apply([&](queue_type& queue) {
            const auto limit = (*profile.synchronize()).queue_limit;
...
auto engine_t::spawn(pool_type& pool) -> void {
    ...
    pool.insert(std::make_pair(
        id.get(),
        slave_t(context, id, *profile.synchronize(), manifest, *loop, std::bind(&engine_t::on_slave_death, shared_from_this(), ph::_1, id.get()))
    ));
...
void
engine_t::assign(slave_t& slave, slave::channel_t& payload) {
...
void
engine_t::despawn(const std::string& id, despawn_policy_t policy) {
...
engine_t::on_handshake(const std::string& id,
...
void
engine_t::on_slave_death(const std::error_code& ec, std::string uuid) {
    if (ec) {
...
void
engine_t::rebalance_events() {
    using boost::adaptors::filtered;

    const auto concurrency = (*profile.synchronize()).concurrency;
