...
void spawn_handle_t::on_terminate(const std::error_code& ec, const std::string& msg) {
    ...
    if (!ec) {
        ...
        COCAINE_LOG_WARNING(_slave->log,
                            "isolation has abnormally terminated slave: [{}]{} - {}",
                            ec.value(),
                            ec.message(),
                            msg);
    }
    // Force slave termination without waiting for heartbeat failure if it was not terminated
    // before.
    _slave->terminate(ec);
}

void spawn_handle_t::on_ready() {
    auto _spawning = spawning.lock();
    auto _slave = slave.lock();
    if (!_spawning || !_slave) {
        COCAINE_LOG_WARNING(log,
                            "isolation notified about spawn, but slave or state is already gone");
        return;
    }
    ...
    try {
        ...
        auto handshaking =
            std::make_shared<state::handshaking_t>(_slave, std::move(_spawning->handle));
...
void spawn_handle_t::on_data(const std::string& data) {
    auto _slave = slave.lock();
    if (_slave) {
