...
struct cgroup_configurator_t : public boost::static_visitor<void> {
    cgroup_configurator_t(cgroup_controller* ptr_, const char* parameter_)
        : ptr(ptr_), parameter(parameter_) {}

    void operator()(const dynamic_t::bool_t& value) const {
        cgroup_add_value_bool(ptr, parameter, value);
    }

    void operator()(const dynamic_t::int_t& value) const {
        cgroup_add_value_int64(ptr, parameter, value);
    }

    void operator()(const dynamic_t::uint_t& value) const {
        cgroup_add_value_uint64(ptr, parameter, value);
    }

    void operator()(const dynamic_t::string_t& value) const {
...
void* init_cgroups(const char* cgroup_name, const dynamic_t& args, logging::logger_t& log) {
    ...
    for (auto type = args.as_object().begin(); type != args.as_object().end(); ++type) {
        ...
        for (auto it = type->second.as_object().begin(); it != type->second.as_object().end();
             ++it) {
            COCAINE_LOG_INFO(log,
                             "setting cgroup controller '{}' parameter '{}' to '{}'",
                             type->first,
                             it->first,
            ...
            try {
                ...
                COCAINE_LOG_ERROR(log,
                                  "unable to set cgroup controller '{}' parameter '{}' - {}",
                                  type->first,
                                  it->first,
                                  e.what());
            }
...
void attach_cgroups(void* cgroup_raw_ptr, logging::logger_t& log) {
    ...
    if ((rv = cgroup_attach_task(cgroup_ptr)) != 0) {
        COCAINE_LOG_ERROR(
            log, "unable to attach the process to a cgroup - {}", cgroup_strerror(rv));
        std::cerr << cocaine::format("unable to attach the process to a cgroup - %s",
                                     cgroup_strerror(rv));
