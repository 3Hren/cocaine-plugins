...
fetcher_t::fetcher_t(asio::io_service& io_context,
    ...
    : handle(std::move(_handle)), logger(std::move(_logger)), watcher(io_context) {}

void fetcher_t::assign(int fd) {
    watcher.apply([&](watcher_type& watcher) { watcher.assign(fd); });

    COCAINE_LOG_DEBUG(logger, "slave has started fetching standard output");
    watch();
}

void fetcher_t::close() {
    watcher.apply([&](watcher_type& watcher) {
        if (watcher.is_open()) {
            ...
            try {
                ...
                COCAINE_LOG_WARNING(
                    logger, "unable to close standard output watcher: {}", err.what());
            }
        }
    });
}

void fetcher_t::watch() {
    COCAINE_LOG_DEBUG(logger, "slave is fetching more standard output");

    watcher.apply([&](watcher_type& watcher) {
        watcher.async_read_some(asio::buffer(buffer.data(), buffer.size()),
                                std::bind(&fetcher_t::on_read, shared_from_this(), ph::_1, ph::_2));
    });
}

void fetcher_t::on_read(const std::error_code& ec, size_t len) {
    switch (ec.value()) {
    case 0:
