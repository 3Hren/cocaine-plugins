...
struct spool_dispatch_t : public dispatch<io::event_traits<io::isolate::spool>::upstream_type> {
    typedef io::protocol<io::event_traits<io::isolate::spool>::upstream_type>::scope protocol;

    spool_dispatch_t(const std::string& name,
                     std::shared_ptr<api::spool_handle_base_t> _spool_handle)
        : dispatch(name), spool_handle(std::move(_spool_handle)) {
        on<protocol::value>(std::bind(&spool_dispatch_t::on_done, this));
        on<protocol::error>(std::bind(
            &spool_dispatch_t::on_error, this, std::placeholders::_1, std::placeholders::_2));
    }
...
struct spawn_dispatch_t : public dispatch<io::event_traits<io::isolate::spawn>::upstream_type> {
    typedef io::protocol<io::event_traits<io::isolate::spawn>::upstream_type>::scope protocol;

    spawn_dispatch_t(const std::string& name,
                     std::shared_ptr<api::spawn_handle_base_t> _spool_handle)
        : dispatch(name), spawn_handle(std::move(_spool_handle)), ready(false) {
        ...
        on<protocol::error>(std::bind(
            &spawn_dispatch_t::on_error, this, std::placeholders::_1, std::placeholders::_2));
    }
...
struct spool_load_t : public api::cancellation_t {
    ...
    spool_load_t(external_t::inner_t& _inner, std::shared_ptr<api::spool_handle_base_t> _handle)
        : inner(_inner), handle(std::move(_handle)) {}
...
struct spawn_load_t : public api::cancellation_t {
    external_t::inner_t& inner;
...
struct external_t::inner_t : public std::enable_shared_from_this<inner_t> {
    context_t& context;
    ...
    inner_t(context_t& _context,
            asio::io_service& _io_context,
            const std::string& _name,
            const std::string& type,
            const dynamic_t& _args)
        : context(_context),
    ...
    void connect() {
        ...
        COCAINE_LOG_INFO(log,
                         "connecting to external isolation daemon to {}",
        ...
        socket->async_connect(endpoint, [=](const std::error_code& ec) {
            if (!ec) {
                ...
                session.apply([&](decltype(session.unsafe())& session) {
                    ...
                    COCAINE_LOG_INFO(
                        log, "processing {} queued spool requests", spool_queue.size());
                    for (auto& load : spool_queue) {
                        load->apply(session);
                    }
                    spool_queue.clear();
                    COCAINE_LOG_INFO(
                        log, "processing {} queued spawn requests", spawn_queue.size());
...
void spool_load_t::apply(std::shared_ptr<session_t> session) {
    stream.apply([&](decltype(stream.unsafe())& stream) {
        if (!cancelled) {
            try {
                stream = session->fork(
                    std::make_shared<spool_dispatch_t>("external_spool_" + inner.name, handle));
...
void spool_load_t::cancel() noexcept {
    stream.apply([&](decltype(stream.unsafe())& stream) {
        if (!cancelled) {
            cancelled = true;
            if (stream) {
                try {
                    ...
                    COCAINE_LOG_WARNING(
                        inner.log, "could not cancel spool request: {}", error::to_string(e));
                }
            }
        }
    });
...
void spawn_load_t::apply(std::shared_ptr<session_t> session) {
    stream.apply([&](decltype(stream.unsafe())& stream) {
        if (!cancelled) {
            try {
                stream = session->fork(
                    std::make_shared<spawn_dispatch_t>("external_spawn_" + inner.name, handle));
                stream->send<io::isolate::spawn>(
                    inner.args, inner.name, path, worker_args, environment);
            } catch (const std::system_error& e) {
...
void spawn_load_t::cancel() noexcept {
    stream.apply([&](decltype(stream.unsafe())& stream) {
        if (!cancelled) {
            cancelled = true;
            if (stream) {
...
external_t::external_t(context_t& context,
                       asio::io_service& io_context,
                       const std::string& name,
                       const std::string& type,
...
std::unique_ptr<api::cancellation_t> external_t::spool(
...
std::unique_ptr<api::cancellation_t> external_t::spawn(
    const std::string& path,
    const api::args_t& worker_args,
    const api::env_t& environment,
    std::shared_ptr<api::spawn_handle_base_t> handler) {
    std::shared_ptr<spawn_load_t> load(
        new spawn_load_t(*inner, std::move(handler), path, worker_args, environment));
    inner->session.apply([&](decltype(inner->session.unsafe())& session) {
        if (session) {
            load->apply(session);
        } else {
